...
// $Id: view.html.php 3495 2011-11-01 21:56:13Z chraneco $
/****************************************************************************************\
**   JoomGallery 2                                                                      **
...
class JoomGalleryViewDetail extends JoomGalleryView
{
  ...
  function display($tpl = null)
  {
    ...
    {
      $this->_doc->setMetaData('title', $image->imgtitle);
    }
      ...
      if($slideshow)
      {
        ...
        foreach($images as $row)
        {
          ...
          if($this->_config->get('jg_showdetailfilesize'))
          {
            ...
            $filesizedtlkb  = JText::_('COM_JOOMGALLERY_DETAIL_KB');
            $filesizedtl    = number_format($filesizedtl/1024, 2, JText::_('COM_JOOMGALLERY_COMMON_DECIMAL_SEPARATOR'), JText::_('COM_JOOMGALLERY_COMMON_THOUSANDS_SEPARATOR'))." ".JText::_('COM_JOOMGALLERY_DETAIL_KB');
            $filesizedtlpx  = JText::_('COM_JOOMGALLERY_DETAIL_PX');
            list($width, $height, $type, $attr) = @getimagesize($this->_ambit->getImg('img_path', $row));
            $filesizedtl .= ' ('.$width.' x '.$height.$filesizedtlpx.')&nbsp';
          }
          else
          {
            $filesizedtl  = '&nbsp;';
          ...
          if($this->_config->get('jg_showoriginalfilesize'))
          {
            ...
            $filesizeorg    = number_format($filesizeorg/1024, 2, JText::_('COM_JOOMGALLERY_COMMON_DECIMAL_SEPARATOR'), JText::_('COM_JOOMGALLERY_COMMON_THOUSANDS_SEPARATOR'))." ".JText::_('COM_JOOMGALLERY_DETAIL_KB');
            $filesizeorgpx  = JText::_('COM_JOOMGALLERY_DETAIL_PX');
            list($width, $height, $type, $attr) = @getimagesize($this->_ambit->getImg('orig_path', $row));
            $filesizeorg .= ' ('.$width.' x '.$height.$filesizeorgpx.')&nbsp';
          }
          else
          {
            $filesizeorg  = '&nbsp;';
    ...
    if(!$slideshow)
    {
      ...
      if($this->_config->get('jg_showrating'))
      {
        ...
        {
          if($this->_config->get('jg_votingonlyonce') && $image->owner == $this->_user->get('id'))
          {
            // Set voting_area to 3 to show only the message in template
            $params->set('show_voting_area', 3);
