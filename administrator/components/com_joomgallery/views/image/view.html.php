...
// $Id: view.html.php 3381 2011-10-07 19:10:00Z aha $
/****************************************************************************************\
**   JoomGallery 2                                                                      **
...
class JoomGalleryViewImage extends JoomGalleryView
{
  ...
  function display($tpl = null)
  {
    ...
    $params_xml = JPATH_COMPONENT.DS.'models'.DS.'forms'.DS.'image_params.xml';
    ...
    if($item->imgthumbname)
    {
      ...
      {
        // We have to look for the image ID fist because the image may have to be output through the script
        $id = $this->getModel()->getIdByFilename($item->imgfilename, $item->thumb_catid, true);
  ...
  function addToolbar()
  {
    ...
    if($this->isNew && ($canDo->get('joom.upload') || count(JoomHelper::getAuthorisedCategories('joom.upload'))))
    {
