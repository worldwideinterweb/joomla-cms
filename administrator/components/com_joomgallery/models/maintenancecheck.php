...
// $Id: maintenancecheck.php 3378 2011-10-07 18:37:56Z aha $
/****************************************************************************************\
**   JoomGallery 2                                                                      **
...
class JoomGalleryModelMaintenancecheck extends JoomGalleryModel
{
  ...
  function check()
  {
    ...
    $this->_db->setQuery('DELETE FROM '._JOOM_TABLE_MAINTENANCE);
    $this->_db->query();

    $this->_db->setQuery('DELETE FROM '._JOOM_TABLE_ORPHANS);
    $this->_db->query();

    $this->_db->setQuery('ALTER TABLE '._JOOM_TABLE_MAINTENANCE.' AUTO_INCREMENT = 1');
    $this->_db->query();

    $this->_db->setQuery('ALTER TABLE '._JOOM_TABLE_ORPHANS.' AUTO_INCREMENT = 1');
    $this->_db->query();
  ...
  function parseFolders()
  {
    ...
    while($continue)
    {
      ...
      if(!$folder)
      {
        ...
        {
          // If we haven't started yet start with the root folder set flag
          $folder = $this->_ambit->get($types[0].'_path');
      ...
      $folder = JPath::clean(rtrim($folder, DS), '/');
  ...
  function createFolderSuggestions()
  {
    $query = $this->_db->getQuery(true);
